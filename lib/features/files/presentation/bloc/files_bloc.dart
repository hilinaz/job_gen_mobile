...
class FilesBloc extends Bloc<FilesEvent, FilesState> {
  ...
  Future<void> _onGetCurrentUserFiles(
    ...
    emit(FilesLoading());
    try {
      final result = await getCurrentUserFiles(event.fileType);

      result.fold(
        (failure) => emit(FilesError(message: failure.message)),
        (files) {
          if (files.isNotEmpty) {
            emit(GetUserFilesState(files: files.map((f) => f as JgFileModel).toList()));
          } else {
            emit(const GetUserFilesState(files: []));
          }
        },
      );
    } catch (e) {
      emit(FilesError(message: 'Failed to load files: $e'));
    }
  }
}
